...
class MCPCell(BaseCell):
    ...
    async def _load_presets(self):
        ...
        presets = [
            ...
            # ── Tier 1/2 additions (awesome-mcp-servers) ──
            ("clock", set(), ResourceLimits(memory_mb=64, timeout=10)),
            ("calculator", set(), ResourceLimits(memory_mb=64, timeout=10)),
            ("text_stats", set(), ResourceLimits(memory_mb=64, timeout=10)),
            ("memory", set(), ResourceLimits(memory_mb=128, timeout=10)),
            ("sequential_thinking", set(), ResourceLimits(memory_mb=128, timeout=10)),
            ("sqlite_query", {ToolCapability.FILESYSTEM}, ResourceLimits(memory_mb=256, timeout=20)),
            ("csv_json", {ToolCapability.FILESYSTEM}, ResourceLimits(memory_mb=256, timeout=15)),
            ("http_request", {ToolCapability.NETWORK}, ResourceLimits(memory_mb=256, timeout=30)),
            ("wikipedia", {ToolCapability.NETWORK}, ResourceLimits(memory_mb=256, timeout=20)),
            ("weather", {ToolCapability.NETWORK}, ResourceLimits(memory_mb=256, timeout=20)),
            ("arxiv", {ToolCapability.NETWORK}, ResourceLimits(memory_mb=256, timeout=25)),
            ("ip_geolocation", {ToolCapability.NETWORK}, ResourceLimits(memory_mb=128, timeout=20)),
            ("dns_lookup", {ToolCapability.NETWORK}, ResourceLimits(memory_mb=128, timeout=15)),
            ("html_to_markdown", {ToolCapability.NETWORK}, ResourceLimits(memory_mb=256, timeout=25)),
            ("clipboard", {ToolCapability.SUBPROCESS}, ResourceLimits(memory_mb=128, timeout=15)),
            ("convert_units", {ToolCapability.NETWORK}, ResourceLimits(memory_mb=128, timeout=20)),
            ("pdf_extract", {ToolCapability.FILESYSTEM}, ResourceLimits(memory_mb=512, timeout=30)),
            ("office_reader", {ToolCapability.FILESYSTEM}, ResourceLimits(memory_mb=512, timeout=30)),
            ("screenshot", {ToolCapability.FILESYSTEM}, ResourceLimits(memory_mb=512, timeout=20)),
            ("process_monitor", set(), ResourceLimits(memory_mb=256, timeout=15)),
            ("qr_code", set(), ResourceLimits(memory_mb=128, timeout=15)),
            ("sympy_math", set(), ResourceLimits(memory_mb=512, timeout=30)),
        ]
